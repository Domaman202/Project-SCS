...
class Laxer(val text: String) {
    ...
    fun parseValue(line: Int): NodeElement {
        val ctx = Context(calcLine(line))
        return ctx.parseVal(line, ctx.calcOffset())
    }
    ...
    inner class Context(var i: Int) {
        ...
        fun parseVal(line: Int, offset: Int): NodeElement {
            skipSpace()
            return if (text[--i] == '"')
        ...
        private fun parseValue(line: Int, offset: Int): NodeElement {
            ...
            return NodeValue(Token.operation(line, "value"), offset, NodeValue.Type.VALUE, value.toString())
        }
